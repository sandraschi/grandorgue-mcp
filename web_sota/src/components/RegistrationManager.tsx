...
export default function RegistrationManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await fetch(`/api/registrations/${editing}`, {
          method: "PUT",
  ...
  const handleEdit = (reg: Registration) => {
    ...
    setOrgan(reg.organ);
    try {
      const parsed: StopEntry[] = JSON.parse(
        typeof reg.stops === "string" ? reg.stops : JSON.stringify(reg.stops),
      );
      setStops(parsed.map((s, idx) => ({ id: idx, cc: s.cc, name: s.name, state: s.state })));
    } catch {
      setStops([]);
    }
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "grandorgue-registrations.json";
    a.click();
  ...
  const addStop = () => {
    setStops((prev) => [
      ...prev,
      {
        ...
        cc: prev.length > 0 ? prev[prev.length - 1].cc + 1 : 0,
        name: "",
        state: true,
      },
    ]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {message && (
        <div className="bg-green-950/40 border border-green-800 text-green-300 px-4 py-2 rounded text-sm flex items-center gap-2">
          <Check size={14} />
          {message}
        </div>
      )}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-3">
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 bg-zinc-800 text-zinc-100 border border-zinc-700 rounded px-3 py-2 text-sm"
          />
          <input
            value={organ}
            onChange={(e) => setOrgan(e.target.value)}
            placeholder="Organ (optional)"
            className="w-48 bg-zinc-800 text-zinc-100 border border-zinc-700 rounded px-3 py-2 text-sm"
          />
        </div>
            ...
            <div key={s.id} className="flex gap-2 items-center">
              ...
              <label className="flex items-center gap-1 text-xs text-zinc-400">
                <input
                  ...
                  onChange={(e) => {
                    const n = [...stops];
                    n[i] = { ...n[i], state: e.target.checked };
                    setStops(n);
                  }}
        ...
        <button
          onClick={handleSave}
          className="px-4 py-2 bg-amber-600 text-black rounded text-sm font-medium hover:bg-amber-500 flex items-center gap-1.5"
        >
          <Save size={14} /> {editing ? "Update" : "Save"} Registration
        </button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {registrations.length === 0 && (
          <div className="text-zinc-600 text-sm text-center py-8">
            No saved registrations yet. Create your first one above.
          </div>
        )}
      </div>
    </div>
