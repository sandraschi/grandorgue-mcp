...
export default function CombinationMemory() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6" data-testid="memory-page">
        ...
        <div className="space-y-3">
          {["Great", "Swell", "Choir", "Pedal"].map((div) => (
            <div key={div} className="flex items-center gap-2">
              <span className="text-sm text-zinc-600 w-12">{div}</span>
              <div className="flex gap-1">
                {DIVISIONALS.map((n) => (
                  <button
                    key={n}
                    onClick={() => trigger(n + 10)}
                    className="w-8 h-8 rounded text-sm bg-zinc-800 text-zinc-500 hover:bg-zinc-700 hover:text-zinc-300 transition-colors"
                  >
                    {n}
                  </button>
                ))}
