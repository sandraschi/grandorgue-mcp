...
export function AppsPage() {
        ...
        list.forEach((a) => {
          fetch(`${API_BASE}/api/apps/health?port=${a.port}`)
            ...
            .catch(() =>
              setHealth((prev) => ({
                ...prev,
                [a.port]: { port: a.port, alive: false },
              })),
            );
        });
      })
    ...
    try {
      ...
      if (j.alive || j.status === "already_running" || j.status === "brought_to_foreground") {
        if (j.status === "brought_to_foreground") {
          if (!j.alive) {
            setError(`${app.id}: Tauri window brought to front — no web UI on :${app.port}`);
          } else {
            setTimeout(
        ...
        const hr = await fetch(`${API_BASE}/api/apps/health?port=${app.port}`)
          .then((x) => x.json())
          .catch(() => null);
        if (hr) setHealth((prev) => ({ ...prev, [app.port]: hr }));
      } else if (
        j.status === "started" ||
        j.status === "start_initiated" ||
        j.status === "tauri_started"
      ) {
        for (let i = 0; i < 15; i++) {
        ...
        if (headless || (app.has_tauri && !app.url)) {
          setError(
            `${app.id}: ${j.error ?? `No web UI — headless MCP/Tauri app. Use its winapp via Starts or: uv run ${app.id}`}`,
          );
        } else {
          window.open(app.url ?? `http://127.0.0.1:${app.port}`, "_blank");
      ...
      if (app.has_tauri) {
        setError(
          `${app.id}: ${e instanceof Error ? e.message : String(e)} — no web UI, try Tauri winapp or start.ps1`,
        );
      } else {
        window.open(app.url ?? `http://127.0.0.1:${app.port}`, "_blank");
  ...
  return (
    <div className="space-y-6" data-testid="apps-page">
      {/* Hero */}
      <div
        className="rounded-2xl border border-border bg-gradient-to-br from-card via-card to-black/20 p-6"
        data-testid="apps-hero"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-[70ch]">
              All installed <span className="font-mono text-sm">sandraschi</span> repos with a
              webapp (from <span className="font-mono text-sm">fleet-registry.json</span> +{" "}
              <span className="font-mono text-sm">WEBAPP_PORTS.md</span>). Click a card to check{" "}
              <span className="font-mono text-sm">:{`{port}`}/health</span> first. If it is up, we
              open it; if a Tauri winapp is already running we bring it to the foreground; otherwise
              we start it via{" "}
              <span className="font-mono text-sm">mcd starts\{`{id}`}-start.bat</span> →{" "}
              <span className="font-mono text-sm">repo\start.ps1</span> → Tauri exe and then open.
            </p>
            <div className="flex flex-wrap gap-2 text-sm font-mono">
              <span className="px-2 py-1 rounded bg-white/5 border border-white/10">
                {fleetTotal} repos total
              </span>
              <span className="px-2 py-1 rounded bg-gh-green/10 border border-gh-green/20 text-gh-green">
                {apps.length} with ports
              </span>
              <span className="px-2 py-1 rounded bg-white/5 border border-white/10">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-medium text-sm flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full shrink-0 ${alive ? "bg-gh-green shadow-[0_0_8px_rgba(34,197,94,0.5)]" : alive === false ? "bg-red-500/60" : "bg-gray-500 animate-pulse"}`}
                      />
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-gh-green/10 border border-gh-green/20 text-gh-green">
                        :{app.port}
                      </span>
                      {app.has_tauri && (
                        <span
                          ...
                          title={
                            app.has_tauri_installed ? "Tauri winapp installed" : "Tauri capable"
                          }
                        >
                ...
                {app.last_commit && (
                  <p className="text-[10px] font-mono text-muted-foreground/60 mt-1">
                    last commit {new Date(app.last_commit).toLocaleDateString()}
                  </p>
                )}
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span
                    className={`text-[10px] font-mono px-2 py-1 rounded border ${alive ? "bg-gh-green/10 border-gh-green/20 text-gh-green" : "bg-white/5 border-white/10 text-muted-foreground"}`}
                  >
          ...
          <div className="divide-y divide-white/5">
            ...
            {filtered.map((app) => {
              ...
              return (
                <div
                  key={app.id}
                  className="grid sm:grid-cols-[1fr_90px_80px_120px_100px] gap-2 px-4 py-3 hover:bg-white/[0.02] items-center"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${alive ? "bg-gh-green" : "bg-red-500/60"}`}
                      />
                  ...
                  <span className="text-sm font-mono text-gh-green">:{app.port}</span>
                  <span className="text-sm font-mono">{app.category}</span>
                  <span className="text-sm font-mono text-muted-foreground">
                    {app.last_commit ? new Date(app.last_commit).toLocaleDateString() : "—"}
                  </span>
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      onClick={() => openApp(app)}
                      disabled={isStarting}
                      className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${alive ? "bg-white text-black" : "bg-amber-500 text-black"} disabled:opacity-60`}
                    >
      ...
      {filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">No apps match filter.</p>
      )}
      ...
      <div className="rounded-lg border border-dashed border-border bg-black/20 p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-muted-foreground leading-relaxed">
          <span className="font-semibold text-foreground">Fleet standard:</span> every{" "}
          <span className="font-mono">Apps</span> page checks{" "}
          <span className="font-mono">:{`{port}`}/health</span> first; if a Tauri winapp is already
          running we bring it to front, otherwise we launch{" "}
          <span className="font-mono">mcd starts\{`{id}`}-start.bat</span> →{" "}
          <span className="font-mono">repo\start.ps1</span> → Tauri exe and then open. Descriptions
          prefer <span className="font-mono">pyproject.toml description</span> (plain) with{" "}
          <span className="font-mono">gh repo</span> fallback — optimize there.
        </div>
      </div>
    </div>
  );
